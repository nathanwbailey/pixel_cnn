...
def preprocess_images(images: np.ndarray) -> tuple[np.ndarray]:
    imgs_int = tf.image.resize(images, (IMAGE_SIZE, IMAGE_SIZE)).numpy()
...
model = build_model(input_shape=(IMAGE_SIZE, IMAGE_SIZE, 3), num_residual_blocks=RESIDUAL_BLOCKS, num_filters=N_FILTERS, pixel_levels=PIXEL_LEVELS)
