...
class ImageGenerator(keras.callbacks.Callback):
    def __init__(self, num_img: int) -> None:
    ...
    def generate(self, temperature: float, pixel_levels: int) -> np.ndarray:
    ...
    def on_epoch_end(self, epoch, pixel_levels: int, logs=None) -> None:
        generated_images = self.generate(temperature=1.0, pixel_levels=pixel_levels)
        display(
            generated_images,
